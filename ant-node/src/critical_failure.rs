    ...
    match error {
        Error::Network(network_error) => {
            ...
            let network_err_str = match network_error {
                ...
                NetworkError::RecordNotStoredByNodes(_) => "RecordNotStoredByNodes".to_string(),
                NetworkError::RecordKindMismatch(_) => "RecordKindMismatch".to_string(),
                NetworkError::InCorrectRecordHeader => "InCorrectRecordHeader".to_string(),
                NetworkError::OperationNotAllowedOnClientRecordStore => {
                    "OperationNotAllowedOnClientRecordStore".to_string()
                }
                NetworkError::FailedToVerifyChunkProof(_) => "FailedToVerifyChunkProof".to_string(),
                NetworkError::NoGraphEntryFoundInsideRecord(_) => {
                    "NoGraphEntryFoundInsideRecord".to_string()
                }
                NetworkError::NotEnoughPeersForStoreCostRequest => {
                    "NotEnoughPeersForStoreCostRequest".to_string()
                }
                NetworkError::NoStoreCostResponses => "NoStoreCostResponses".to_string(),
                ...
                NetworkError::BehaviourErr(_) => "BehaviourErr".to_string(),
                NetworkError::EventChannelFailure(_) => "EventChannelFailure".to_string(),
            };
        ...
        Error::NodeEventParsingFailed => "NodeEventParsingFailed".to_string(),
        Error::Bootstrap(error) => match error {
            ...
            ant_bootstrap::Error::FailedToParseUrl => "FailedToParseUrl".to_string(),
            ant_bootstrap::Error::Io(_) => "BootstrapIoError".to_string(),
            ant_bootstrap::Error::Json(_) => "BootstrapJsonError".to_string(),
            ant_bootstrap::Error::Http(_) => "BootstrapHttpError".to_string(),
            ant_bootstrap::Error::LockError => "BootstrapLockError".to_string(),
        },
        ...
        Error::TerminateSignalReceived(_) => "TerminateSignalReceived".to_string(),
        Error::Tokio => "Tokio".to_string(),
    }
}
