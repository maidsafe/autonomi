...
use crate::networking::endpoint_str;
...
impl ReachabilityIssue {
    fn retryable(&self) -> bool {
        matches!(
            self,
            ReachabilityIssue::NoDialBacks | ReachabilityIssue::NotEnoughDialBacks { .. }
        )
    }
}
...
impl ReachabilityCheckSwarmDriver {
    ...
    pub(crate) async fn detect(mut self) -> Result<ReachabilityStatus, NetworkError> {
        ...
        let mut dial_check_interval = tokio::time::interval(std::time::Duration::from_secs(2));
    ...
    fn trigger_dial(&mut self) {
        while self.dial_manager.can_dial_new_peer() {
            let mut addr = match self.dial_manager.get_next_contact() {
                Ok(Some(addr)) => addr,
                Ok(None) => {
                    trace!("More addresses are being fetched. Waiting for them to arrive.");
                    return;
                }
                Err(_) => {
                    error!(
