...
pub(crate) struct Dialer {
    ...
    pub(super) ongoing_dial_attempts: HashMap<PeerId, DialState>,
...
impl DialManager {
    ...
    /// If Ok(None) is returned, it means that more addresses are being fetched.
    /// If Err is returned, it means that there are no more addresses to dial.
    pub(crate) fn get_next_contact(&mut self) -> Result<Option<Multiaddr>> {
        let addr = self.bootstrap.next_addr()?;
        match addr {
            Some(addr) => match multiaddr_get_p2p(&addr) {
                Some(peer_id) => {
                    if peer_id == self.self_peer_id {
                        warn!("Next contact address is our own address: {addr}. Skipping it.");
                ...
                None => {
                    warn!("Next contact address does not contain a peer id: {addr}. Skipping it.");
                    self.get_next_contact()
                }
            },
            None => Ok(None),
    ...
    pub(crate) fn has_dialing_completed(&self) -> bool {
        if self.dialer.ongoing_dial_attempts.len() < get_majority(MAX_CONCURRENT_DIALS) {
            match self.bootstrap.is_addr_queue_empty() {
                Some(empty) => {
                    if empty {
                        info!(
                            "Dialing has completed because: the address queue is empty && we do not have majority dial attempts in progress."
                        );
                        return true;
                    }
                }
                None => {
                    warn!(
                        "Dialing not completed because the Bootstrap struct is actively fetching more addresses to dial."
                    );
                    return false;
                }
            }
