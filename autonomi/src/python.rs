...
// #[pyclass(name = "ANTNetwork")]
// #[derive(Debug, Clone)]
// pub struct PyANTNetwork {
//     inner: ANTNetwork,
// }

//TODOv: add the 12 methods of PyANTNetwork
...
impl PyBootstrapCacheConfig {
    ...
    #[staticmethod]
    fn default_config(local: bool) -> PyResult<Self> {
        match BootstrapCacheConfig::default_config(local) {
    ...
    fn with_cache_path(&self, path: PathBuf) -> Self {
        Self {
            inner: self.inner.clone().with_cache_path(path),
        }
    }
    ...
    fn cache_file_path(&self) -> PathBuf {
        self.inner.cache_file_path.clone()
    }
...
impl PyInitialPeersConfig {
    ...
    /// Whether to disable fetching peers from mainnet contacts
    #[getter]
    fn get_disable_mainnet_contacts(&self) -> bool {
        self.inner.disable_mainnet_contacts
    }

    #[setter]
    fn set_disable_mainnet_contacts(&mut self, value: bool) {
        self.inner.disable_mainnet_contacts = value;
    }
    ...
    fn fetch_bootstrap_addrs<'a>(
        ...
        future_into_py(py, async move {
            match inner_config.get_addrs(rust_config, count).await {
                Ok(addrs) => Ok(addrs
                    .into_iter()
                    .map(|addr| addr.to_string())
                    .collect::<Vec<String>>()),
    ...
    /// Get bootstrap peers with their bootstrap addresses sorted by the failure rate.
    /// The peer with the lowest failure rate will be the first in the list.
    #[pyo3(signature = (config=None, count=None))]
    fn fetch_bootstrap_addr_details<'a>(
        &self,
        py: Python<'a>,
        config: Option<PyBootstrapCacheConfig>,
        count: Option<usize>,
        ...
        future_into_py(py, async move {
            match inner_config.get_bootstrap_addr(rust_config, count).await {
                Ok(bootstrap_addrs) => {
                    // Since failure_rate is private, we'll just return the addresses as strings
    ...
    /// Read multiaddresses from the ANT_PEERS environment variable
    #[staticmethod]
    fn read_addr_from_env() -> Vec<String> {
        InitialPeersConfig::read_addr_from_env()
            .into_iter()
            .map(|addr| addr.to_string())
            .collect()
    }
    ...
    fn get_bootstrap_cache_path(&self) -> PyResult<Option<PathBuf>> {
        self.inner.get_bootstrap_cache_path().map_err(|e| {
            PyRuntimeError::new_err(format!("Failed to get bootstrap cache path: {e}"))
        })
    }
...
impl PyTransactionConfig {
    /// Creates a new TransactionConfig with the specified maximum fee per gas
    #[new]
    fn new(max_fee_per_gas: u128) -> Self {
        Self {
            inner: TransactionConfig::new(max_fee_per_gas),
        }
    }
    ...
    fn default() -> Self {
        Self {
            inner: TransactionConfig::default(),
        }
    }

    /// Get the maximum fee per gas
    #[getter]
    fn get_max_fee_per_gas(&self) -> u128 {
        self.inner.max_fee_per_gas
    }

    /// Set the maximum fee per gas
    #[setter]
    fn set_max_fee_per_gas(&mut self, value: u128) {
        self.inner.max_fee_per_gas = value;
    }

    /// Return a string representation of the transaction config
    fn __str__(&self) -> String {
        format!(
