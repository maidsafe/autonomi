        ...
        while !re_check_queue.is_empty() {
            if let Some(node) = re_check_queue.pop_back() {
                ...
                let is_reachability_check_ongoing =
                    Self::is_reachability_check_ongoing(Arc::clone(&node))
                        .await
                        .unwrap_or(false);
                ...
                info!(
                    "Reachability check is not ongoing for {}, checking if the node is connected to the network",
                    node.read().await.service_name
                );
