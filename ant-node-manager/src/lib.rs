...
pub async fn refresh_node_registry(
    ...
    for node in &mut node_registry.nodes {
        ...
        let mut service =
            NodeService::new(node, Box::new(rpc_client.clone()), Box::new(metrics_client));
        ...
        if is_local_network {
            ...
            match service.metrics_action.get_node_metrics().await {
                ...
                Err(_) => {
                    debug!(
                        "Local node {} is not running or metrics are not available",
                        service.service_data.service_name
                    );
...
mod tests {
    ...
    async fn upgrade_should_retain_the_reachability_check_flag() -> Result<()> {
        ...
        assert!(service_manager.service.service_data.reachability_check);
