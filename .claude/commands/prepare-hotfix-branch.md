# Prepare Hotfix Branch for Release Candidate
...
**CRITICAL**:
* The current branch must not have any outstanding changes. If this is the case, stop and ask the
  user to commit those changes.
* The current branch name should be in the form `rc-YYYY.X.Y-hotfixZ`, where `X`, `Y` and `Z` are
  integers. If this is not the case, warn the user and get them to create a new branch in this form
  before proceeding.
...
messages to the #releases channel. The channel has ID `C0A3ZL5F8PK`. All messages should be prefixed
with `[timestamp] <release-year>.<release-month>.<release-cycle>.<release-cycle-counter>: `, which
you can obtain from the `release-cycle-info` file. The timestamp should be in the ISO-style format.

## Phase 1: Preparing the new release candidate

Read the current `release-cycle-info` file to obtain the package version. It is in the following
form:
```
<release-year>.<release-month>.<release-cycle>.<release-cycle-counter>
```

Increment the `<release-cycle-counter>` value by one and then update the `release-cycle-info` and `release_info.rs` files accordingly.

Post a message to Slack indicating that the release candidate process is beginning.

## Phase 2: Determine the merged PRs

Use `git` to determine the PR numbers between this branch and the last stable release, which is
denoted by a tag that can be obtained using the `gh` tool. Use `gh` to query for the latest release on
the `maidsafe/autonomi` repository, and the tag associated with that release is the one you need to
determine the PR numbers.

We use merge commits, so the PR numbers are readily available.
...
So these are grouped by the author. All the PR information should be obtainable using the `gh` tool.
...
release list. After that, we can then move to the next phase.
