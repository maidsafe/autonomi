...
# This script removes RC versions of the `ant` binary from S3.
# It is used to clean up after a release candidate test.
#
# Usage: ./remove-s3-ant-rc-binaries.sh <version>
# Example: ./remove-s3-ant-rc-binaries.sh 0.3.1-rc.1
#
...
echo "Removing ant binary version $version from S3..."

for arch in "${architectures[@]}"; do
  zip_filename="ant-${version}-${arch}.zip"
  tar_filename="ant-${version}-${arch}.tar.gz"
