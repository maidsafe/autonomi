...
#[tokio::test]
async fn test_network_contacts_url_validation() -> Result<()> {
    let _guard = LogBuilder::init_single_threaded_tokio_test("cli_integration_tests", false);
    ...
    let args = PeersArgs {
        first: false,
        addrs: vec![],
        network_contacts_url: vec!["invalid:url".to_string()],
        local: false,
        disable_mainnet_contacts: true,
        ignore_cache: true,
    ...
    let result = args.get_bootstrap_addr(None, None).await;
