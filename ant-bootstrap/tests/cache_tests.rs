...
use ant_bootstrap::{BootstrapCacheConfig, BootstrapCacheStore, ContactsFetcher, PeersArgs};
...
async fn test_max_addrs_per_peer() -> Result<()> {
    ...
    for i in 1..=4 {
        let addr: Multiaddr =
            format!("/ip4/127.0.0.{}/udp/8080/quic-v1/p2p/{peer_id}", i).parse()?;
        cache_store.add_addr(addr);
    }
...
async fn test_first_flag_behavior() -> Result<()> {
    ...
    // Create PeersArgs with first=true and other conflicting options
    let args = PeersArgs {
        first: true,
