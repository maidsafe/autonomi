...
/// Faster registry polling when nodes are in transitional states (Starting/Updating).
...
const NODE_REGISTRY_STARTING_UPDATE_INTERVAL: Duration = Duration::from_secs(5);
...
impl Status<'_> {
    ...
    fn try_update_node_registry(&mut self, force_update: bool) -> Result<()> {
        ...
        let interval = if has_transitioning_nodes {
            NODE_REGISTRY_STARTING_UPDATE_INTERVAL
        } else {
