...
use super::{keyboard::KeySequence, test_helpers::TestAppBuilder};
use crate::{
    action::Action,
    app::App,
    focus::FocusTarget,
...
impl Journey {
    ...
    pub async fn assert_current_screen(&self, assertion: &ScreenAssertion) -> Result<()> {
        match assertion {
            ScreenAssertion::ExactScreen(_expected_lines) => {
                println!("Exact screen assertion (simplified for testing)");
            }
            ScreenAssertion::ContainsText(text) => {
                println!("Asserting screen contains text: '{text}'");
            }
...
impl JourneyBuilder {
    ...
    pub async fn new_with_nodes(name: &str, node_count: usize) -> Result<Self> {
