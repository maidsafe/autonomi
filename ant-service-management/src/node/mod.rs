...
use crate::{
    control::ServiceControl, error::Result, metric::MetricsAction, rpc::RpcActions,
...
impl ServiceStateActions for NodeService {
    ...
    async fn build_upgrade_install_context(
        ...
        if let Some(metrics_port) = service_data.metrics_port {
            ...
            error!("Metrics port not available during upgrade_install_context. Make sure to call ServiceStateActions::set_metrics_port_if_not_set before building context");
        }
    ...
    async fn wait_until_started(&self) -> Result<()> {
        ...
        info!("Reachability check completed for {service_name}. Now waiting for the node to connect to the network");
