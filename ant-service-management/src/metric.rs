...
impl Default for NodeInfoMetrics {
    fn default() -> Self {
        Self {
            peer_id: String::from("unknown"), //initialization
            pid: 0,
    ...
    pub fn get_node_info_from_metadata_extended(
        ...
        for sample in scrape.samples.iter() {
            for (key, value) in sample.labels.iter() {
                match key.as_str() {
                    "peer_id" => node_info.peer_id = value.to_string(),
...
impl MetricActions for MetricClient {
    async fn node_info(&self) -> Result<NodeInfo> {
        ...
        Ok(NodeInfo {
            peer_id: PeerId::from_str(node_info.peer_id.as_str())
                .map_err(|_| Error::MetricServicePeerIdParseError(node_info.peer_id.clone()))?,
            pid: node_info.pid,
