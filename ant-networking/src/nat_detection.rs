...
impl NatDetectionSwarmDriver {
    pub fn new(swarm: Swarm<NatDetectionBehaviour>, initial_contacts: Vec<Multiaddr>) -> Self {
        let swarm = swarm;
        let initial_bootstrap = InitialBootstrap::new(
            initial_contacts.clone(),
            Some(NAT_DETECTION_MAX_SUCCESSFUL_DIALS),
        );
    ...
    fn on_upnp_result(&mut self) {
        let mut initial_bootstrap = InitialBootstrap::new(
            self.initial_contacts.clone(),
            Some(NAT_DETECTION_MAX_SUCCESSFUL_DIALS),
        );
